...
common::Error::Error(const stim::DemInstruction& error) {
  ...
  // Detectors in the set are already sorted order, which we need so that there is
  // a unique canonical representative for each set of detectors.
...
stim::DetectorErrorModel common::dem_from_counts(
  ...
  if (orig_dem.count_errors() != error_counts.size()) {
    throw std::invalid_argument(
        "Error hits array must be the same size as the number of errors in the original DEM.");
  }
  stim::DetectorErrorModel out_dem;
  size_t ei = 0;
