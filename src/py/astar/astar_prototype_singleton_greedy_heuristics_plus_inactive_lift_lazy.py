...
Supported heuristic choices:
    ...
    lifted_sweep      1-pass inactive bounds transferred to plain_sweep
    lifted_exact_lp   1-pass inactive bounds transferred to exact_lp
...
This works for all of the above heuristics because each returns a feasible
singleton-budget vector y, and projecting that y to a child by keeping prices
on detectors that remain active and zeroing newly active detectors is still a
feasible child singleton-budget point.
"""
...
class GreedySingletonHeuristicDecoder:
    ...
    def _apply_inactive_lift(self, y: np.ndarray, active_dets: np.ndarray, available_errs: np.ndarray) -> float:
        """Modifies y IN PLACE to increase values using a post-processing dual slack transfer."""
        slacks = np.zeros(self.num_errors, dtype=np.float64)
        available_indices = np.flatnonzero(available_errs)
        
        # Calculate initial slacks
        for ei in available_indices:
            ei = int(ei)
            slacks[ei] = self.weights[ei]
            for d in self.error_detectors[ei]:
                if active_dets[d]:
                    slacks[ei] -= y[d]
                    
        active_list = np.flatnonzero(active_dets)
        # Sort descending to attack the heaviest y-values first
        order = sorted((int(d) for d in active_list), key=lambda d: y[d], reverse=True)
        
        for d in order:
            incident_eis = [int(ei) for ei in self.d2e[d] if available_errs[ei]]
            if not incident_eis:
                continue
                
            min_s = min(slacks[ei] for ei in incident_eis)
            
            # If the detector is bottled-necked, try to transfer slack from inactive neighbors
            if min_s < 1e-9:
                blocking_eis = [ei for ei in incident_eis if slacks[ei] < 1e-9]
                for ei in blocking_eis:
                    for d_inact in self.error_detectors[ei]:
                        if not active_dets[d_inact]:
                            siblings = [int(j) for j in self.d2e[d_inact] if available_errs[j] and j != ei]
                            if not siblings:
                                continue
                            delta = min(slacks[j] for j in siblings)
                            if delta > 1e-9:
            ...
            # Re-evaluate the bottleneck and lift if space was created
            new_min_s = min(slacks[ei] for ei in incident_eis)
            if new_min_s > 1e-9:
                y[d] += new_min_s
                for ei in incident_eis:
                    slacks[ei] -= new_min_s
                    
        return float(y[active_dets].sum())
    ...
    def compute_support_based_heuristic(
        ...
        *,
        ...
        h_name = name or self.heuristic_name
        
        support_data = self.build_support_data(dets, available)

        if h_name in {"lifted_sweep", "lifted_exact_lp"}:
            if h_name == "lifted_sweep":
                safe_val, safe_y = self.heuristic_plain_sweep(support_data)
            else:
                safe_val, safe_y = self.heuristic_exact_lp(support_data)
                
            if safe_y is None:
                return INF, None
                
            lifted_val = self._apply_inactive_lift(safe_y.copy(), dets, available)
            # Return max to guarantee it is >= base heuristic, and safe_y to keep projection valid
            return max(lifted_val, safe_val), safe_y

        return self.evaluate_named_heuristic(support_data, h_name)
    ...
    def report_root_heuristics(self, dets: np.ndarray, errs: np.ndarray, blocked_errs: np.ndarray) -> List[Tuple[str, float]]:
        available = self._available_errors(errs, blocked_errs)
        support_data = self.build_support_data(dets, available)
        names = ["plain", "asc_deg", "desc_plain", "plain_sweep", "best_of_two", "best_of_three", "exact_lp", "lifted_sweep", "lifted_exact_lp"]
        out: List[Tuple[str, float]] = []
        saved_lp_calls = self.lp_calls

        for name in names:
            if name in {"lifted_sweep", "lifted_exact_lp"}:
                if name == "lifted_sweep":
                    safe_val, safe_y = self.heuristic_plain_sweep(support_data)
                else:
                    safe_val, safe_y = self.heuristic_exact_lp(support_data)
                if safe_y is None:
                    ...
                    lifted_val = self._apply_inactive_lift(safe_y.copy(), dets, available)
                    out.append((name, max(lifted_val, safe_val)))
            else:
    ...
    def _maybe_refine_node(self, state: SearchState) -> Tuple[SearchState, bool]:
        ...
        if self.heuristic_name in {"exact_lp", "lifted_exact_lp"} and new_value + 1e-7 < state.h_cost:
            raise AssertionError(
                f"Exact LP value {new_value} below stored projected value {state.h_cost}."
            )
        ...
        if previous_source == "projected":
            self.projected_nodes_refined += 1
        if abs(new_value - state.h_cost) <= HEURISTIC_EPS:
            state.y_prices = new_y
        state.refined = True
        if self.verbose_search:
    ...
    def decode(self, shot_dets: np.ndarray, det_beam: float = INF) -> DecodeResult:
        ...
        if root_refined and self.heuristic_name != "plain":
            ...
            if eager_y is None or math.isinf(eager_h):
                return DecodeResult(
                    success=False,
                    errs=errs0,
                    residual_dets=dets0,
        ...
        while heap:
            ...
            for ei in self.d2e[min_det]:
                ...
                if child_y is None or math.isinf(child_h):
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--heuristic",
        choices=["plain", "asc_deg", "desc_plain", "plain_sweep", "best_of_two", "best_of_three", "exact_lp", "lifted_sweep", "lifted_exact_lp"],
        default="best_of_two",
