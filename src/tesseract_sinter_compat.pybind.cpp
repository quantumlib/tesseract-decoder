...
struct TesseractSinterCompiledDecoder {
    ...
    py::array_t<uint8_t> decode_shots_bit_packed(
        ...
        if (bit_packed_detection_event_data.shape(1) != (py::ssize_t)num_detector_bytes) {
            throw std::invalid_argument("Input array's second dimension does not match num_detector_bytes.");
        }
...
struct TesseractSinterDecoder {
    ...
    void decode_via_files(
        ...
        uint64_t num_dets,
        uint64_t num_obs,
        const py::object& dem_path,
        const py::object& dets_b8_in_path,
        const py::object& obs_predictions_b8_out_path,
        const py::object& tmp_dir
    ) {
...
void pybind_sinter_compat(py::module& m) {
    ...
    .def_readwrite("num_detectors", &TesseractSinterCompiledDecoder::num_detectors, R"pbdoc(The number of detectors in the decoder's underlying DEM.)pbdoc")
    .def_readwrite("num_observables", &TesseractSinterCompiledDecoder::num_observables, R"pbdoc(The number of logical observables in the decoder's underlying DEM.)pbdoc");
    ...
    py::class_<TesseractSinterDecoder>(m, "TesseractSinterDecoder", R"pbdoc(
        ...
        .def("decode_via_files", &TesseractSinterDecoder::decode_via_files,
             py::kw_only(),
             py::arg("num_shots"),
             py::arg("num_dets"),
             py::arg("num_obs"),
             py::arg("dem_path"),
             py::arg("dets_b8_in_path"),
             py::arg("obs_predictions_b8_out_path"),
             py::arg("tmp_dir"), R"pbdoc(
        ...
        .def(py::self == py::self, R"pbdoc(Checks if two TesseractSinterDecoder instances are equal.)pbdoc")
        .def(py::self != py::self, R"pbdoc(Checks if two TesseractSinterDecoder instances are not equal.)pbdoc");

    // Define the sinter_decoders function, which is the entry point for sinter.
    m.def("sinter_decoders", []() {
        py::dict result;
