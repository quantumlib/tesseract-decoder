...
public:
    ...
    struct ComponentDecoder {
        ...
        std::map<Hyperedge, size_t> symptom_to_error_index;
...
public:
    static const std::vector<std::vector<size_t>>& get_pass_schedule(const MultiPassTesseractDecoder& decoder) {
    ...
    static const TesseractDecoder& get_component_decoder(const MultiPassTesseractDecoder& decoder, size_t i) {
        return *decoder.component_decoders[i].decoder;
    }
    static const std::vector<size_t>& get_modified_component_indices(const MultiPassTesseractDecoder& decoder) {
        return decoder.modified_component_indices;
    }
    static void print_full_trace(
        MultiPassTesseractDecoder& mp_decoder,
        const stim::Circuit& circuit,
        const std::vector<uint64_t>& detections,
        const std::vector<int>& true_obs);
};
